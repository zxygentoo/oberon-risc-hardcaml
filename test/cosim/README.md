...
| file | role |
|---|---|
| `<unit>_dump.ml` | the per-unit dumper: drives the Hardcaml port over a stimulus set, dumps a trace to stdout (the stimulus source). `fp_dump` serves all three FP units (`Risc5.Fp_<unit>`, chosen by the `<name>` arg), dumping `"x y [u v] z cycles"` (`cycles` = stall length); the serial units have their own — `spi_dump`/`rs232t_dump`/`rs232r_dump`/`ps2_dump` dump a per-cycle hex trace, `vid_dump`/`mouse_dump` a two-clock / bidirectional one. Each sorts next to its `<unit>.cpp` |
