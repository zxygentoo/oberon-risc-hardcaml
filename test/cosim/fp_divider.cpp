...
// Verilator on each stimulus dumped by test/cosim/dump_fp, and assert RTL z == port z. Exit 0
// iff the Hardcaml port is bit-exact to FPDivider.v over the whole stimulus set.
...
// hold inputs, run, drain until stall drops (S 0->26), read z, release run for one cycle
...
int main(int argc, char** argv) {
  ...
  fclose(f);
  printf("fp_divider co-sim: %ld stimuli, %ld mismatch\n", n, mismatch);
  if (mismatch == 0) printf("==> Hardcaml Fp_divider is bit-exact to FPDivider.v.\n");
  delete dut;
