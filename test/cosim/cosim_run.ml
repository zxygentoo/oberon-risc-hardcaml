(* Parallel RTL-fidelity co-sim runner (replaces the old run.sh + run-core.sh).
   ...
   - The CPU core: a whole-boot capture-and-replay — capture the core's per-cycle I/O over
     a real Oberon boot (test/core_dump.exe, ~90 s, cached as a ~400 MiB trace) and replay
     it through RISC5.v + submodules, reporting the first cycle our port diverges (skips
     the 2-cycle reset transient; see core.cpp).
...
let run_core name ~rtls ~extra_v ~cpp ~top ~skip =
  ...
  let cap_ok =
    ...
    then (
      ...
      Printf.printf "[1/3] capturing core boot I/O -> %s (~90 s) ...\n" trace;
