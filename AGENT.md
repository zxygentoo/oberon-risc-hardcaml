...
| **8** *(stretch)* | `test/formal` / `@formal`, two modes: **combinational** (`hardcaml_of_verilog` import + `hardcaml_verify` `Sec` + z3) and **sequential** (emit our Verilog → yosys `equiv_induct`, k-induction). **Working**: both shifters (z3) + the Multiplier and Divider (yosys, FFs paired by name) proven ≡ their `.v`. FP follows the Multiplier pattern; the ALU (inline, no standalone `.v`) + full core go in-situ when `RISC5.v` is imported | z3 `Unsat` / yosys all-`$equiv`-proven |
...
6. **Formal** — *prove* (not sample) our port equivalent to the reference `.v`; the exhaustive form
   ...
   *emit* our Verilog (`Rtl`) and prove equivalence inside yosys (`equiv_make` pairs flip-flops by
   ...
   Multiplier and Divider proven ≡ their `.v`; FP follows the same pattern. The ALU has **no** standalone
