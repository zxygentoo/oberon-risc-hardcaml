...
build is `Framebuf` (`boards/nexys-4/framebuf.ml`): the framebuffer **shadowed in on-chip BRAM**, the
...
oberon-risc-hardcaml/
  ...
  test/                   ← tests; `test_fp_*` + `test_cpu_lockstep` + `test_rom` (ROM guard,
    ...
    boards/<target>/      ← board-SoC integration tests + the board bench, mirroring
                            boards/<target>/: the PSRAM boot checkpoint + visual golden +
...
it bakes the design ROM (`Risc5.Rom`), so the whole `boards/<target>/` is emu-free. Tests
